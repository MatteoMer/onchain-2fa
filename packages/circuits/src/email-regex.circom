...
// regex: onchain-2fa: send this mail to verify your transaction @ 0x[a-fA-F0-9]{40}
template Test(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		state_changed[i] = MultiOR(54);
		eq[0][i] = IsEqual();
		eq[0][i].in[0] <== in[i];
		eq[0][i].in[1] <== 102;
		and[0][i] = AND();
		and[0][i].a <== states[i][54];
		...
		multi_or[0][i].in[14] <== eq[28][i].out;
		multi_or[0][i].in[15] <== eq[29][i].out;
		multi_or[0][i].in[16] <== eq[7][i].out;
		multi_or[0][i].in[17] <== eq[30][i].out;
		multi_or[0][i].in[18] <== eq[10][i].out;
		multi_or[0][i].in[19] <== eq[31][i].out;
		multi_or[0][i].in[20] <== eq[32][i].out;
		multi_or[0][i].in[21] <== eq[0][i].out;
		and[24][i].b <== multi_or[0][i].out;
		states[i+1][25] <== and[24][i].out;
		state_changed[i].in[24] <== states[i+1][25];
		and[25][i] = AND();
		and[25][i].a <== states[i][0];
		and[25][i].b <== eq[3][i].out;
	...
	for (var i = 0; i <= num_bytes; i++) {
		final_state_result.in[i] <== states[i][30];
	}
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][30] * (1 - is_consecutive[msg_bytes-i][1]) + is_consecutive[msg_bytes-i][1];
